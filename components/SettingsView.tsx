...
const SettingsView: React.FC<Props> = ({ 
  ...
  const [activeTab, setActiveTab] = useState<'profile' | 'preferences' | 'exchange' | 'company'>('profile');
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 relative z-10">
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-indigo-200 font-medium max-sm:text-sm">Configure global identity, currency, and company branding assets.</p>
            </div>
          </div>
          <div className="flex bg-white/5 p-1.5 rounded-2xl border border-white/5 overflow-x-auto max-w-full">
            {['profile', 'preferences', 'exchange', 'company'].map(tab => (
              (tab !== 'company' || isSuperAdmin) && (
                <button 
                  key={tab} 
                  onClick={() => setActiveTab(tab as any)} 
                  className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === tab ? 'bg-white text-slate-900' : 'text-slate-400 hover:text-white'}`}
                >
                  {tab}
                </button>
              )
      ...
      {activeTab === 'exchange' && (
        <div className="bg-white dark:bg-slate-900 p-10 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm space-y-8">
              ...
              <div className="flex items-center gap-4">
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="pl-9 pr-4 py-2 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500/20"
                    value={rateSearch}
                    onChange={(e) => setRateSearch(e.target.value)}
                  />
                </div>
                <button onClick={() => setLocalRates(exchangeRates)} className="text-[10px] font-black uppercase text-indigo-600 hover:underline flex items-center gap-1">
             ...
             {filteredCurrencies.map(curr => (
               <div key={curr.code} className="bg-slate-50 dark:bg-slate-800 p-5 rounded-3xl border border-slate-200 dark:border-slate-700 space-y-3">
                 <div className="flex items-center justify-between">
                    ...
                    <span className="px-2 py-0.5 bg-indigo-100 dark:bg-indigo-900/50 text-indigo-600 dark:text-indigo-300 text-[9px] font-black rounded-md">{curr.code}</span>
                 </div>
                 <div className="flex items-center gap-3">
                   <div className="flex-1 relative">
                     ...
                     <input 
                       type="number"
                       step="0.0001"
                       className="w-full pl-16 pr-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900 text-sm font-black outline-none focus:ring-4 focus:ring-indigo-500/10"
           ...
           <div className="pt-6 border-t border-slate-100 dark:border-slate-800 flex justify-end">
              <button onClick={handleSave} disabled={isSaving} className="bg-indigo-600 text-white px-10 py-4 rounded-2xl font-black uppercase text-xs hover:bg-indigo-700 shadow-xl shadow-indigo-500/30 flex items-center gap-2 active:scale-95 disabled:opacity-50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="md:col-span-1 space-y-6">
            <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col items-center text-center overflow-hidden">
              <div className="w-24 h-24 rounded-3xl bg-indigo-100 dark:bg-indigo-900/30 flex items-center justify-center text-indigo-600 dark:text-indigo-400 text-4xl font-black border-4 border-white dark:border-slate-800 shadow-xl mb-4 uppercase">
                {profile?.email?.substring(0, 2)}
              </div>
              {/* Fix: Restricted email text to prevent overflow using truncate and width constraint */}
              <h3 className="text-xl font-black mb-1 truncate w-full px-2" title={profile?.email || ''}>{profile?.email}</h3>
              <div className="flex items-center gap-2 mb-8">
                <span className="px-3 py-1 bg-indigo-50 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-300 rounded-full text-[10px] font-black uppercase tracking-widest border border-indigo-100 dark:border-indigo-800">
                  {currentRole?.name}
                </span>
                <span className="px-3 py-1 bg-emerald-50 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-300 rounded-full text-[10px] font-black uppercase tracking-widest border border-emerald-100 dark:border-indigo-800">
                  {profile?.status}
                </span>
              </div>
              ...
              <div className="w-full space-y-4">
                <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700">
                   ...
                   <div 
                     ...
                     className={`w-11 h-6 rounded-full p-1 cursor-pointer transition-all ${isDarkMode ? 'bg-indigo-600' : 'bg-slate-300'}`}
                   >
          ...
          <div className="md:col-span-2 space-y-6">
            <div className="bg-white dark:bg-slate-900 p-10 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm space-y-8">
              <div className="flex items-center gap-3 border-b border-slate-100 dark:border-slate-800 pb-6">
                 ...
                 <h4 className="text-lg font-black">Authorization Context</h4>
              </div>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Access Tier</label>
                  <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 font-bold">
        ...
        <div className="bg-white dark:bg-slate-900 p-10 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm space-y-12">
           <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
              <div className="space-y-6">
                 <div className="flex items-center gap-3">
                    ...
                    <h4 className="text-lg font-black tracking-tight">Regional Settings</h4>
                 </div>
                 ...
                 <div className="space-y-6">
                    <div className="space-y-2">
                       ...
                       <div className="relative">
                          ...
                          <select 
                            className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl py-4 pl-12 pr-4 text-sm font-bold outline-none focus:ring-4 focus:ring-indigo-500/10 appearance-none"
              ...
              <div className="space-y-6">
                 <div className="flex items-center gap-3">
                    ...
                    <h4 className="text-lg font-black tracking-tight">Notification Pulse</h4>
                 </div>
           ...
           <div className="pt-10 border-t border-slate-100 dark:border-slate-800 flex justify-end">
              <button 
                onClick={handleSave}
                disabled={isSaving}
                className="bg-indigo-600 text-white px-10 py-4 rounded-2xl font-black uppercase text-xs hover:bg-indigo-700 shadow-xl shadow-indigo-500/30 transition-all flex items-center gap-2 active:scale-95 disabled:opacity-50"
              >
      ...
      {activeTab === 'company' && isSuperAdmin && (
        <div className="bg-white dark:bg-slate-900 p-10 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm space-y-10 animate-in slide-in-from-right-4 duration-300">
           <div className="flex items-center gap-3">
              ...
              <h4 className="text-lg font-black tracking-tight">Enterprise Identity Configuration</h4>
           </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div className="space-y-8">
                 <div className="space-y-2">
                    ...
                    <input 
                      type="text"
                      className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl py-4 px-6 text-sm font-black outline-none focus:ring-4 focus:ring-indigo-500/10"
                      value={companyName}
                            ...
                            <>
                              {isPDFLetterhead ? (
                                <div className="flex flex-col items-center justify-center space-y-2">
                                  ...
                                  <span className="text-xs font-black uppercase text-indigo-600">PDF Asset Registered</span>
                                </div>
                              ) : (
                                <img src={pendingLetterhead} alt="Letterhead Preview" className="w-full h-full object-contain" />
                              )}
                              <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                                 <div className="p-2 bg-white rounded-full text-indigo-600 shadow-xl"><Upload size={20}/></div>
                                 <button 
                 ...
                 <div className="space-y-2">
                    ...
                    <textarea 
                      className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl py-4 px-6 text-sm font-medium h-32 outline-none focus:ring-4 focus:ring-indigo-500/10"
              ...
              <div className="space-y-6">
                 <div className="space-y-2">
                    ...
                    <textarea 
                      className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl py-4 px-6 text-sm font-medium h-64 outline-none focus:ring-4 focus:ring-indigo-500/10"
                      value={pendingTerms}
           ...
           <div className="pt-10 border-t border-slate-100 dark:border-slate-800 flex justify-end">
              <button 
                onClick={handleSave} 
                disabled={isSaving}
                className="bg-indigo-600 text-white px-10 py-4 rounded-2xl font-black uppercase text-xs hover:bg-indigo-700 shadow-xl shadow-indigo-500/30 transition-all active:scale-95 disabled:opacity-50 flex items-center gap-2"
              >
                {isSaving ? <RefreshCw className="animate-spin" size={16}/> : <Save size={16}/>}
